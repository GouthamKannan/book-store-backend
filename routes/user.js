...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    if (userDetails.length == 1) {
      ...
      res.cookie("user_name", userDetails[0].user_name)
      return res.status(200).json({
        success: true,
...
router.get("/logout", async (req, res) => {
  return res.clearCookie("session_id").clearCookie("user_name")
    .status(200)
...
router.get("/verify_email", async(req, res) => {
  try {
    ...
    if(response.length==1)
    {
        ...
        res.redirect(process.env.UI_HOST + "/home/email_verified")
    }
    else {
